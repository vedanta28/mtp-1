...
\begin{document}
	\begin{tikzpicture}[->,shorten >=1pt,auto,node distance=3.35cm,
		...
		(l0) edge [out=35, in=150]     node[above, xshift=0em, text width=10cm, align=center] {$\mathsf{x\_up}$ $\wedge$ $\mathsf{max\_x\_limit}$} (lv)
		
		(l0) edge [out=8, in=175]     node[above, xshift=0em, text width=10cm, align=center] {$\mathsf{x\_down}$ $\wedge$ $\mathsf{min\_x\_limit}$} (lv)
		
		(l0) edge [out=-8, in=185]     node[below, xshift=0em, text width=10cm, align=center] {$\mathsf{y\_down}$ $\wedge$ $\mathsf{min\_y\_limit}$} (lv)

		(l0) edge [out=-35, in=-150]     node[below, xshift=0em, text width=10cm, align=center] {$\mathsf{y\_up}$ $\wedge$ $\mathsf{max\_y\_limit}$} (lv)
